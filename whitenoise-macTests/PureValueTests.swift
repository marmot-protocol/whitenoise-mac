...
struct PureValueTests {
    ...
    @MainActor
    @Test func anchoredProjectionAppendsSameSecondUpsertAfterRuntimeBlock() async throws {
        // Regression for whitenoise-mac#451: loaded windows preserve MarmotKit runtime order
        // within a second; projection inserts must append to that block, not binary-search by id.
        func message(id: String, timelineAt: UInt64) -> MessageItem {
            MessageItem(
                id: id,
                senderName: "sender",
        ...
        // Runtime order at t=100 is "runtime-b" then "runtime-a" (not ascending id order).
        let store = MessageTimelineStore.loaded(with: [
            message(id: "runtime-b", timelineAt: 100),
            message(id: "runtime-a", timelineAt: 100),
        ])

        _ = store.applyProjection(
            upserts: [message(id: "runtime-0", timelineAt: 100)],
            removals: [],
            anchoredToNewest: true,
            windowLimit: 10
        )

        #expect(store.messages.map(\.id) == ["runtime-b", "runtime-a", "runtime-0"])
    }

    @MainActor
    @Test func detachedWindowAcceptsSameSecondUpsertAtHeadDespiteIdOrder() async throws {
        // Regression for whitenoise-mac#451: detached gating must compare second-granular
        // timelineAt only; a same-second head upsert must not be dropped because its id sorts
        // after messages.last.id. Strictly newer timestamps still stay outside the window.
        func message(id: String, timelineAt: UInt64) -> MessageItem {
            MessageItem(
                id: id,
                senderName: "sender",
        ...
        let store = MessageTimelineStore.loaded(with: [
            message(id: "L", timelineAt: 95),
            message(id: "M", timelineAt: 100),
            message(id: "N", timelineAt: 100),
        ])

        _ = store.applyProjection(
            upserts: [message(id: "Z", timelineAt: 100)],
            removals: [],
            anchoredToNewest: false,
            windowLimit: 10
        )

        #expect(store.messages.map(\.id) == ["L", "M", "N", "Z"])

        _ = store.applyProjection(
            upserts: [message(id: "future", timelineAt: 101)],
            removals: [],
            anchoredToNewest: false,
            windowLimit: 10
        )
        ...
        #expect(store.messages.map(\.id) == ["L", "M", "N", "Z"])
        #expect(!store.containsMessage(id: "future"))
    }
